...
# 注1：慕课网课程中的rating_norm[i, idx] 的计算代码有问题，感谢在评论下解惑的大牛们(￣▽￣)~*
...
#定义保存信息的路径
filename = 'ml-latest-small\movie_tensorboard'
#把信息保存在文件当中
...
sortedResult = predicts[:, int(userNo-1)].argsort()[::-1]
# idx用于保存已经推荐了多少部电影
idx = 0
...
for i in sortedResult:
    ...
    print('评分： %.2f, 电影名： %s' % (predicts[i, int(userNo-1)], movies_df.iloc[i]['title']))
    idx += 1
    if idx == 20:
        break

print("\n=====================================\n")
print("Tensor Flow1.7 CPU Running Time taken:", datetime.now() - startTime)
