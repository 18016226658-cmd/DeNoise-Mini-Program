...
def generate_histories():
    ...
    with app.app_context():
        ...
        if not users:
            print('❌ 没有找到用户，请先运行 generate_real_users.py 生成用户')
            return
        
        print('=' * 60)
        print(f'开始为 {len(users)} 个用户生成历史记录')
        print('=' * 60)
        ...
        print(f'\n✅ 成功生成 {total_histories} 条历史记录！')
        
        # 显示统计信息
        print('\n历史记录统计：')
        total = History.query.count()
        complete = History.query.filter_by(factorization_level='complete').count()
        partial = History.query.filter_by(factorization_level='partial').count()
        failed = History.query.filter_by(factorization_level='failed').count()
        ...
        for level in ['normal', 'vip', 'svip']:
            users_with_level = User.query.filter_by(level=level).all()
            if users_with_level:
                user_ids = [u.id for u in users_with_level]
                count = History.query.filter(History.user_id.in_(user_ids)).count()
                level_name = {'normal': '普通用户', 'vip': 'VIP用户', 'svip': 'SVIP用户'}[level]
                print(f'  {level_name}: {count} 条记录')
