...
#     for i in range(0,len(lines)-1,2):           # 正式转换所有行
    for i in range(0,2000,2):                   # 测试用，仅仅处理前 100行 记录，产生 50对聊天数据
...
for i in range(df.shape[0]):
    n=df.index
    k =random.randint(1, 10)   # 产生 1、2、3、4、5 。。。9  其中5/9 为男性  4/9为 女性
    if   k <=4:     # 1、2、3、4    4/9为 女性
    ...
    # 下面为新添加聊天模型名称（原没用的字段）
    Mx = random.randint(0, 3)
    MxName = "文心一言"
    if Mx==1:
        MxName="GPT"
    df.loc[i, 'MxName'] = MxName  # 随机生成聊天模型名称
...
for i in range(0, 10):
    ...
    print(df.loc[i, 'MxName'])

# 保存到excel 中
df.to_excel('./Dialo-2024-1.xlsx', index=False)
